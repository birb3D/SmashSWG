--Automatically generated by SWGEmu Spawn Tool v0.12 loot editor.

legendary_strange_armor_segment_padded = {
	minimumLevel = 0,
	maximumLevel = -1,
	customObjectName = "Experimental Padded Armor Segment",
	directObjectTemplate = "object/tangible/component/armor/armor_segment_padded_advanced.iff",
	craftingValues = {
		{"armor_special_type",-1,-1,0},
		{"armor_effectiveness",25,45,10},
		{"armor_special_effectiveness",10,45,10},
		{"armor_integrity",300,3000,10},
		{"armor_health_encumbrance",12,1,0},
		{"armor_action_encumbrance",20,1,0},
		{"armor_mind_encumbrance",10,1,0},
		{"useCount",3,8,0},
	},
	customizationStringNames = {},
	customizationValues = {}
}

addLootItemTemplate("legendary_strange_armor_segment_padded", legendary_strange_armor_segment_padded)