...
function MerchantSystem:startShopping(pObject)
     CreatureObject(pObject):setScreenPlayState(MerchantSystem.states.active, MerchantSystem.screenplayName)
end


function MerchantSystem:endShopping(pObject)
    CreatureObject(pObject):removeScreenPlayState(MerchantSystem.states.active, MerchantSystem.screenplayName)
    CreatureObject(pObject):setScreenPlayState(MerchantSystem.states.complete, MerchantSystem.screenplayName)
end


function MerchantSystem:resetStates(pObject)
    CreatureObject(pObject):removeScreenPlayState(MerchantSystem.states.active, MerchantSystem.screenplayName)
    CreatureObject(pObject):removeScreenPlayState(MerchantSystem.states.complete, MerchantSystem.screenplayName)
end
...
function MerchantSystem:adjustPrice(conversingPlayer, cost, relationsTable)
	...
	for lc = 1, #relationsTable , 1 do
		...
		if pGhost ~= nil then
			local playerStanding = PlayerObject(pGhost):getFactionStanding(relationsTable[lc].name)
		...
		if pGhost ~= nil then
			...
			if (playerStanding ~= -9000) then -- Ignore special case price adjust standing
				if (playerStanding < relationsTable[lc].npcStanding and relationsTable[lc].npcStanding > 0) then
					CreatureObject(conversingPlayer):sendSystemMessage("Hint: Increase your " .. MerchantSystem:firstToUpper(relationsTable[lc].name) .. " faction and speak with the NPC again.")
				...
				elseif (relationsTable[lc].npcStanding < 0 and (relationsTable[lc].npcStanding + playerStanding) > 0) then
					CreatureObject(conversingPlayer):sendSystemMessage("Hint: Decrease your " .. MerchantSystem:firstToUpper(relationsTable[lc].name) .. " faction and speak with the NPC again.")
...
function MerchantSystem:nextConvoScreenInnards(conversationTemplate, conversingPlayer, selectedOption, relationsTable, goodsTable)
	...
	if ( conversation ~= nil ) then
		...
		if ( lastConversationScreen == nil ) then
			...
			local isShopping = CreatureObject(conversingPlayer):hasScreenPlayState(MerchantSystem.states.active, MerchantSystem.screenplayName)
			local checkingOut = CreatureObject(conversingPlayer):hasScreenPlayState(MerchantSystem.states.complete, MerchantSystem.screenplayName)
